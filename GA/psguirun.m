...
%   $Revision: 1.14.4.3 $  $Date: 2005/05/31 16:30:19 $
...
[fun,X0,aineq,bineq,aeq,beq,LB,UB,nonlcon,randstate,randnstate,errP] = psguiReadProblem(hproblem);
...
warning off;
try
    [x,fval,e,output] = patternsearch(fun,X0,aineq,bineq,aeq,beq,LB,UB,nonlcon,options);
    psResults.x = x;
    psResults.fval = fval;

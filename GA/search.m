function [successSearch,nextIterate,FunEval] = search(FUN,nonlcon,Xin,searchtype,CompleteSearch,Iterate, ...
    Successdir,pollorder,MeshSize,scale,ToL,A,LB,UB,IndIneqcstr,IndEqcstr,Iter,FunEval,maxFun,type, ...
    NotVectorizedSearch,Cache,cachetol,cachelimit,searchFcnArg,conFcnArg,objFcnArg)
...
% 		
% 		TYPE: This flag is passed to the SEARCH routines, indicating that the
...
%   $Revision: 1.12.4.2 $  $Date: 2005/05/31 16:30:23 $
%   Rakesh Kumar
...
if ~isa(searchtype,'function_handle') 
    ...
    if ischar(searchtype) 
        ...
        if any(strcmpi(searchtype,{'gpspositivebasisnp1', 'gpspositivebasis2n','madspositivebasisnp1','madspositivebasis2n','positivebasisnp1','positivebasis2n'})) && ~strcmpi(searchtype,'none') 
            [successSearch,nextIterate,FunEval,successdir] = poll(FUN,nonlcon,Xin,searchtype,CompleteSearch,pollorder,Iterate, ...
             Successdir,MeshSize,scale,ToL,A,LB,UB,IndIneqcstr,IndEqcstr,(Iter+1),FunEval,maxFun,type, ...
             NotVectorizedSearch,Cache,cachetol,cachelimit,conFcnArg,objFcnArg);
             ...
             searchOptions = struct('completesearch',CompleteSearch,'meshsize',MeshSize,'iteration',Iter,'scale',scale,'indineqcstr',IndIneqcstr, ...
                    'indeqcstr',IndEqcstr,'problemtype',type,'notvectorized',NotVectorizedSearch,'cache',Cache, ...
                    'cachetol',cachetol,'cachelimit',cachelimit,'constr',nonlcon,'constrArg',{conFcnArg});
            try
               [successSearch,nextIterate,FunEval] = feval(searchtype,FUN,Xin,Iterate,ToL,A,LB,UB,FunEval,maxFun,searchOptions,objFcnArg,searchFcnArg{:});

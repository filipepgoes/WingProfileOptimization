...
if ~isempty(nonlcon)
    [constr,conFcnArgs] = isFcn(nonlcon);
    if(~isempty(constr)) && (isa(constr,'inline') || isa(constr,'function_handle'))
        options.NonconFcn = constr;
        options.(['NonconFcn' 'Args']) = conFcnArgs;
    else
        msg = sprintf('The constraint function must be a function handle.');
        error('gads:CONSTRVALIDATE:FUNCTIONHANDLEORCELL:needHandleOrInline',msg);
    end
else
    options.NonconFcn = [];

...
%   $Revision: 1.10.4.3 $  $Date: 2005/05/31 16:29:54 $
...
switch lower(choice)
    case 'ga'
        ...
        code = [code sprintf('nonlconFunction = %s;\n',value2RHS(obj.nonlcon))];
        ...
        code = [code sprintf('%s\n','%%Run GA')];    
        code = [code sprintf('[X,FVAL,REASON,OUTPUT,POPULATION,SCORES] = ga(fitnessFunction,nvars,Aineq,Bineq,Aeq,Beq,LB,UB,nonlconFunction,options);')];
    ...
    case 'ps'
       ...
       code = [code sprintf('%s\n','%%Run PATTERNSEARCH')];
       code = [code sprintf('[X,FVAL,EXITFLAG,OUTPUT] = patternsearch(objectiveFunction,X0,Aineq,Bineq,Aeq,Beq,LB,UB,nonlconFunction,options);')];
    otherwise
        code = '';
end
